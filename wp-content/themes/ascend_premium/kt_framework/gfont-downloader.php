...
function kadence_gfont_unhook_google() {
	global $ascend;
	if ( isset( $ascend['load_google_fonts_locally'] ) && 1 == $ascend['load_google_fonts_locally'] ) {
		wp_dequeue_style('redux-google-fonts-ascend');
	}
}
...
function kadence_gfont_local_css() {
	...
	if ( isset( $ascend['load_google_fonts_locally'] ) && 1 == $ascend['load_google_fonts_locally'] ) {
		...
		$options_fonts = array('font_site_title', 'font_site_tagline', 'mobile_font_site_title', 'mobile_font_site_tagline', 'font_shop_title', 'font_h1', 'font_h2', 'font_h3', 'font_h4', 'font_h5', 'font_subtitle', 'font_p', 'font_primary_menu', 'font_secondary_menu', 'dropdown_menu_font_size', 'font_mobile_menu');
		$load_gfonts = array();
			...
			if ( isset( $ascend[$options_key] ) && true == $ascend[$options_key]['google'] ) {
				if( isset( $load_gfonts[ sanitize_key( $ascend[$options_key]['font-family'] ) ] ) ) {
					...
					if(isset($ascend[$options_key]['font-weight']) && !empty($ascend[$options_key]['font-weight']) ) {
						$load_gfonts[ sanitize_key( $ascend[$options_key]['font-family'] ) ]['font-weight'][$ascend[$options_key]['font-weight']] = $ascend[$options_key]['font-weight'];
					}
					if(isset($ascend[$options_key]['font-style']) && !empty($ascend[$options_key]['font-style']) && !is_numeric($ascend[$options_key]['font-style'])  ) {
						$load_gfonts[ sanitize_key( $ascend[$options_key]['font-family'] ) ]['font-style'][$ascend[$options_key]['font-style']] = $ascend[$options_key]['font-style'];
					}
					if(isset($ascend[$options_key]['subsets']) && !empty($ascend[$options_key]['subsets']) ) {
						$load_gfonts[ sanitize_key( $ascend[$options_key]['font-family'] ) ]['subsets'] = $ascend[$options_key]['subsets'];
					}
				} else {
				...
				if ( 'font_p' == $options_key ) {
					$load_gfonts[ sanitize_key( $ascend[$options_key]['font-family'] ) ]['font-style']['italic'] = 'italic';
					$load_gfonts[ sanitize_key( $ascend[$options_key]['font-family'] ) ]['font-style']['normal'] = 'normal';
					$load_gfonts[ sanitize_key( $ascend[$options_key]['font-family'] ) ]['font-weight']['400'] = '400';
					$load_gfonts[ sanitize_key( $ascend[$options_key]['font-family'] ) ]['font-weight']['700'] = '700';
				}
			}
		}
...
class Kadence_GFont_Downloader {
	...
	private function get_fonts() {
		if ( is_null( self::$all_fonts ) ) {
			...
			$path = trailingslashit( get_template_directory() ) . 'kt_framework/gfont-downloader-json.php';
